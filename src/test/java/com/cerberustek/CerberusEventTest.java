...
public class CerberusEventTest {

    public static void main(String... args) {
        CerberusRegistry.getInstance().registerService(CerberusEvent.class, new CerberusEventImpl());
        CerberusRegistry.getInstance().requestStart(CerberusEvent.class);
        CerberusEvent event = CerberusRegistry.getInstance().getService(CerberusEvent.class);

        event.addListener(new TestListener());
        event.executeFullEIT(new TestEvent());
    }
}
